...
The transformations implemented:
	...
	- Export additional files to be included in cLuaState:
		...
		- TypeDescription specializations for custom classes to allow generc push and pop implementations
...
local function OutputLuaStateHelpers(a_Package)
	...
	for idx, item in ipairs(a_Package) do
		...
		if (mt.classtype == "class") then
			table.insert(types, {type = item.type, name = item.name, lname = item.lname})
		end
	end
	table.sort(types,
	...
	do
		...
		f:write("// Provides a forward declaration for each class exported to the Lua API.\n")
		f:write("\n\n\n\n\n")
	...
	-- Output the TypeDesciptions:
	do
		local f = assert(io.open("LuaState_TypeDescs.inc", "w"))
		f:write("\n// LuaState_TypeDescs.inc\n\n// This file is generated along with the Lua bindings by ToLua. Do not edit manually, do not commit to repo.\n")
		...
		f:write("\t/** Utility struct that provides a way to get the type string for a type.\n")
		f:write("\tMust be a struct in order to support partial specialization and must not be inside a class (gcc / clang complain about that).\n")
		f:write("\tThe general declaration must not be used by the code, all types need a specialization returning the correct type. */\n")
		f:write("\ttemplate <typename T>\n")
		f:write("\tstruct TypeDescription\n\t{\n")
		f:write("\t\t/* If compiler complains on the following line, you need to make sure that the type\n")
		f:write("\t\tpassed as the template parameter to this structure has an appropriate TypeDescription specialization.\n")
		f:write("\t\tUsually these are created automatically for all API classes by ToLua++ in the LuaStateParams_TypeDescs.inc file.\n")
		f:write("\t\tFor basic types, the specializations are below. */\n")
		f:write("\t\tstatic const char * desc() = delete;\n\t};\n\n\n\n\n")
		for _, item in ipairs(types) do
			f:write("\ttemplate <> struct TypeDescription<" .. item.type .. "> { static const char * desc() { return \"" .. item.type .. "\"; } };\n")
